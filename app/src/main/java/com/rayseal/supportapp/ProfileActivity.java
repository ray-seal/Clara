...
public class ProfileActivity extends AppCompatActivity {
    ...
    private Switch showDisplayNameSwitch, showActualNameSwitch, showProfilePicSwitch, showCoverPhotoSwitch, showSupportCategoriesSwitch, showBioSwitch, showContactSwitch, showStatsSwitch;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        private void setEditing(boolean editing) {
            ...
            showBioSwitch.setEditing(editing);
            ...
            cancelBtn.setVisibility(editing ? View.VISIBILITY : View.GONE);
            editBtn.setVisibility(editing ? View.VISIBILITY : View.VISIBLE);
        ...
        private void pickImage(int reqCode_ {
        ...
        private void fillViews(Profile profile) {
            ...
            if (profile.profilePictureUrl != null && !profile.profilePictureUrl.isEmpty()) Glide.with(this).load(profile.profilePictureUrl).into(profilePicView);
            else
            profilePicView.setImageResource(R.drawable.ic_person);

            if (coverPhotoUrl != null && !profile.coverPhotoUrl.isEmpty()) Glide.with(this).load(profile.coverPhotoUrl).into(coverPhotoView);
            else
            ...
            memberSinceText.setText("Member since: " + new java.text.SimpleDateFormat("dd-MM-yyyy").format(new Date(profile.memberSince)));
        ...
        private void uploadImage(Uri uri, String filename, OnImageUploadListener listener) {
            ...
            ref.putFile(uri).addOnSuccessListener(taskSnapshot -> ref.getDownloadUrl().addOnSuccessListener(uri1 -> listener.onSuccess(uri1.toString()))).addOnFailureListener(e -> {
        ...
        private void saveProfileToFirestore(Profile p) {
            db.collection("profiles").document(p.uid).set(p).addOnSuccessListener(unused -> {
        ...
        protected void onActivityResult(int requestCode, int resultCode, Intent data) {
            ...
            if (resultCode == Activity.RESULT_OK && data != null && data.getData() != null) {
                if (requestCode == PICK_PROFILE_PIC) {
                    ...
                    profilePicView.setImangeUri(profilePicUri);
                } else if (requestCode == PICK_COVER_PHOTO) {
                    coverPhotoUri = data.getData();
                    coverPhotoView.setImangeUri(coverPhotoUri);
                }
            }
        }
        ...
        private void showCrisisDialog() {
            new AlertDialog.Builder(this).setTitle("Need Help Now?").setMessage("If you are in a crisis, please reach out to a trusted contact or call emergency services.").setPositiveButton("OK", null).show();
        }

        private void openSettings() {
            // Start settings activity when set up
